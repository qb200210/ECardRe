...
public class ECardSQLHelperCachedShares extends SQLiteOpenHelper {
    ...
    private static final String TABLE_CREATE =
                "CREATE TABLE " + TABLE_NAME_CACHEDSHARES + " (" +
                KEY_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                KEY_STORED + " INTEGER, " +
                KEY_PARTYA + " TEXT, " +
                KEY_PARTYB + " TEXT)";

    private static final String[] COLUMNS = {KEY_ID, KEY_STORED, KEY_PARTYA, KEY_PARTYB};
	...
	public List<OfflineDataCachedShares> getData(String column, String value){
		...
		Cursor cursor = db.query(TABLE_NAME_CACHEDSHARES, COLUMNS, " " + column + " = ?", new String[] {value}, null, null, null, null);
		if(cursor != null){
	...
	public int updataData(OfflineDataCachedShares olData){
		...
		int i = db.update(TABLE_NAME_CACHEDSHARES, values, KEY_PARTYB+" = ?", new String[]{olData.getPartyB()});
		db.close();
	...
	public void deleteData(OfflineDataCachedShares olData){
		SQLiteDatabase db = this.getWritableDatabase();
		db.delete(TABLE_NAME_CACHEDSHARES, KEY_PARTYB+" =?", new String[] {olData.getPartyB()});
		db.close();
